...
until the `APP_BUILDER_TOKEN_ENCRYPTION_KEY` (token flow) or the three
`APP_BUILDER_GITLAB_*` values (gitlab.com OAuth flow) are set in `.env`; see the
comments there, and the setup guide at https://docs.understand.tech.
